...
This document serves as a detailed record of the specific engineering practices, workflows, and optimization techniques we employed while training the Ceview SBERT multi-class classifier. Our goal was to ensure that the entire training pipeline adhered to industry-level standards for data science and machine learning.
...
3. **The Final Test**: Only after the Champion Model is selected do we evaluate it against the completely unseen 10% Test Set. We calculate the `test_loss` and test metrics **exactly once**. By never checking the Test Set during experimentation, we completely avoid **"Data Leakage"** (the cardinal sin of accidentally training the model to memorize the test data).

## 2. Training Optimization & Guardrail Techniques
During our experimentation, we explored and implemented several advanced techniques to squeeze the best performance out of the model while protecting it from overfitting:
...
* **Early Stopping (Patience)**: Instead of blindly forcing the model to train for all 20 epochs, the model constantly monitors the validation loss. If the `val_loss` fails to improve for 5 consecutive epochs, the training halts early. This saves significant GPU computing power and prevents catastrophic overfitting.
...
## 3. Data Integrity & Stratification
Our dataset contains roughly 1,383 examples with a mild class imbalance (a 2.73 ratio between the largest and smallest classes). To handle this without artificially injecting synthetic data:
* **Stratified 80/10/10 Splitting**: Instead of randomly splitting the data, we used strict stratification. This guarantees that the exact class proportions (e.g., 20% Theme Parks) are mirrored perfectly across the Training, Validation, and Testing sets. This ensures that even the smallest minority classes have guaranteed representation during the Final Test, dramatically reducing statistical variance.
* **Macro F1 Evaluation**: Instead of relying purely on standard "Accuracy" (which can be easily tricked by class imbalances), we evaluate our models using the `Macro F1 Score`. This metric treats every class equally, forcing the model to perform well on minority classes to achieve a high score.

## 4. Reproducibility (Random Seeds) for High-Efficiency Testing
...
## 5. Observability and Code Parity
To ensure a smooth transition between local development and cloud training:
* **Weights & Biases (W&B) Tracking**: We integrated industry-standard MLOps observability. Every run streams its loss curves, hardware utilization, and hyperparameters live to a cloud dashboard. This allows us to visually compare 5-10 different attempts side-by-side to easily pick the champion model.
* **Environment Parity (Absolute Paths)**: We structured the codebase to be entirely decoupled. Using `os.path.abspath`, our scripts dynamically resolve their file paths. This means the exact same code runs flawlessly whether it is executed in a local Windows IDE or inside a Linux-based Google Colab notebook, completely eliminating "file not found" errors during cloud deployment.

## 6. Current Architecture Status (SBERT Frozen)
